...
from tg_bot_aggregator.domain.bots.schemas import BotCreate, BotRead, BotUpdate
...
async def update_bot(
    bot_id: int, payload: BotUpdate, uow: UnitOfWork = Depends(get_uow)
) -> object:
    repo = uow.bots
...
async def delete_bot(bot_id: int, uow: UnitOfWork = Depends(get_uow)) -> None:
    deleted = await uow.bots.delete(bot_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="bot not found")
    await uow.commit()
